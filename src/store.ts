...
export function seedDemoData(): void {
  ...
  setItems(KEYS.academicYears, [year]);

  // Add teachers
  const teachers: Teacher[] = [
    { id: 't1', name: 'أ/ محمد أحمد', phone: '01012345678', specialization: 'رياضيات', salary: 0, commission: 75, created_at: new Date().toISOString() },
    { id: 't2', name: 'أ/ أحمد علي', phone: '01023456789', specialization: 'فيزياء', salary: 0, commission: 70, created_at: new Date().toISOString() },
    { id: 't3', name: 'أ/ سارة محمود', phone: '01034567890', specialization: 'كيمياء', salary: 0, commission: 80, created_at: new Date().toISOString() },
  ];
  setItems(KEYS.teachers, teachers);

  const groups: Group[] = [
    { id: 'g1', name: 'مجموعة الرياضيات - أولى ثانوي', teacher_id: 't1', teacher: 'أ/ محمد أحمد', schedule: 'السبت والثلاثاء', days: ['السبت', 'الثلاثاء'], time: '16:00', max_students: 25, fees: 300, academic_year_id: 'year1', created_at: new Date().toISOString() },
    { id: 'g2', name: 'مجموعة الفيزياء - ثانية ثانوي', teacher_id: 't2', teacher: 'أ/ أحمد علي', schedule: 'الأحد والأربعاء', days: ['الأحد', 'الأربعاء'], time: '18:00', max_students: 20, fees: 350, academic_year_id: 'year1', created_at: new Date().toISOString() },
    { id: 'g3', name: 'مجموعة الكيمياء - ثالثة ثانوي', teacher_id: 't3', teacher: 'أ/ سارة محمود', schedule: 'الاثنين والخميس', days: ['الاثنين', 'الخميس'], time: '17:00', max_students: 20, fees: 400, academic_year_id: 'year1', created_at: new Date().toISOString() },
  ];
  setItems(KEYS.groups, groups);

  const studentNames = [
    'أحمد محمد علي', 'محمد إبراهيم حسن', 'يوسف عبدالله سعيد', 'عمر خالد أحمد', 'علي حسين محمد',
    'مريم أحمد عبدالرحمن', 'فاطمة محمد سيد', 'نورهان عادل حسين', 'سارة خالد محمود', 'هدى عبدالله إبراهيم',
    'كريم محمود حسن', 'طارق سعيد أحمد', 'حسام علي محمد', 'ياسين أحمد عبدالله', 'أمير محمد خالد'
  ];
  ...
  const students: Student[] = studentNames.map((name, i) => ({
    ...
    parent_phone: `011${String(Math.floor(Math.random() * 100000000)).padStart(8, '0')}`,
    grade: grades[i % 3],
  ...
  // Create enrollments - some students in multiple groups
  const enrollments: StudentEnrollment[] = [];
  // All students enrolled in their primary group
  students.forEach(s => {
    enrollments.push({
      id: `enr_${s.id}_${s.group_id}`,
      student_id: s.id,
      group_id: s.group_id,
      enrolled_at: new Date().toISOString(),
    });
  });
  // Some students enrolled in additional groups (multi-group)
  // Student 1 (أحمد محمد علي) also in physics group
  enrollments.push({ id: 'enr_s1_g2', student_id: 's1', group_id: 'g2', enrolled_at: new Date().toISOString() });
  // Student 2 (محمد إبراهيم حسن) also in chemistry group  
  enrollments.push({ id: 'enr_s2_g3', student_id: 's2', group_id: 'g3', enrolled_at: new Date().toISOString() });
  // Student 6 (مريم أحمد) also in math group
  enrollments.push({ id: 'enr_s6_g1', student_id: 's6', group_id: 'g1', enrolled_at: new Date().toISOString() });
  // Student 11 (كريم محمود) also in physics group
  enrollments.push({ id: 'enr_s11_g2', student_id: 's11', group_id: 'g2', enrolled_at: new Date().toISOString() });
  setItems(KEYS.enrollments, enrollments);

  const today = new Date().toISOString().split('T')[0];
  const attendanceRecords: Attendance[] = students.slice(0, 10).map((s, i) => ({
    id: `a${i + 1}`,
    student_id: s.id,
    group_id: s.group_id,
    date: today,
    status: i % 5 === 0 ? 'absent' as const : 'present' as const,
  }));
  setItems(KEYS.attendance, attendanceRecords);

  const currentMonth = new Date().toISOString().slice(0, 7);
  const payments: Payment[] = students.slice(0, 8).map((s, i) => {
  ...
  const subscriptions: StudentSubscription[] = students.map((s, i) => ({
    id: `sub${i + 1}`,
    student_id: s.id,
    group_id: s.group_id,
    month: currentMonth,
    status: i < 8 ? 'paid' as const : 'pending' as const,
    amount: groups.find(g => g.id === s.group_id)?.fees || 300,
    payment_id: i < 8 ? `p${i + 1}` : undefined,
    due_date: `${currentMonth}-10`,
  }));
  setItems(KEYS.subscriptions, subscriptions);

  const exams: Exam[] = [
    { id: 'e1', title: 'امتحان شهر يناير - رياضيات', group_id: 'g1', date: today, total_score: 100 },
  ];
  setItems(KEYS.exams, exams);

  const examResults: ExamResult[] = students.filter(s => s.group_id === 'g1').map((s, i) => ({
    id: `er${i + 1}`,
    exam_id: 'e1',
    student_id: s.id,
    score: Math.floor(Math.random() * 40) + 60,
  }));
  setItems(KEYS.examResults, examResults);

  const expenses: Expense[] = [
    { id: 'exp1', category: 'rent', description: 'إيجار المقر', amount: 5000, date: today, notes: '' },
    { id: 'exp2', category: 'electricity', description: 'فاتورة كهرباء', amount: 800, date: today, notes: '' },
  ];
  setItems(KEYS.expenses, expenses);
